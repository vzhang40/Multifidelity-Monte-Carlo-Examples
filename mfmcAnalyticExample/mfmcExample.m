...
figure(1); clf(1);
x = linspace(0, 5, 100);
plot(x, f1(x), 'Color', [0 0.4470 0.7410], "LineStyle","-")
hold on
plot(x, f2(x), 'Color', [0.8500 0.3250 0.0980], "LineStyle","-")
legend("High Fidelity $f^{(1)}$", "Low Fidelity $f^{(2)}$", "Interpreter", "latex")
xlabel("x", "Interpreter", "latex")
ylabel("Model Values", "Interpreter", "latex")
title("Models", "Interpreter", "latex")
...
mfmc_MSE_anal = zeros(length(p), 1);
mfmc_MSE = zeros(length(p), 1);
mc_MSE_anal = zeros(length(p), 1);
mc_MSE = zeros(length(p), 1);
...
for n = 1:length(p)
    ...
    % Mean Square Error Analytical
    mfmc_MSE_anal(n) = sigma(1)^2*(1-rho(2)^2)*p(n)./(m_MFMC(1)^2*w(1));
    mc_MSE_anal(n) = sigma(1)^2/m_MC;

    % MSE experimental 
    mfmc_MSE(n) = (1./R).*sum((mu_true - s_MFMC).^2);
    mc_MSE(n) = (1./R).*sum((mu_true - s_MC).^2);
end
...
plot(p, mfmc_means, 'Color', [0.8500 0.3250 0.0980], "LineStyle","-")
plot(p, mc_means, 'Color', [0 0.4470 0.7410], "LineStyle","-")
...
% Analytical MSE Corollary 3.5
loglog(p, mfmc_MSE_anal, 'Color', [0.8500 0.3250 0.0980], "LineStyle","-")
hold on
loglog(p, mc_MSE_anal, 'Color', [0 0.4470 0.7410], "LineStyle","-")
loglog(p, mfmc_MSE, 'Color', [0.8500 0.3250 0.0980], "Marker","o", "LineStyle","none")
loglog(p, mc_MSE, 'Color', [0 0.4470 0.7410], "Marker","o", "LineStyle","none")
